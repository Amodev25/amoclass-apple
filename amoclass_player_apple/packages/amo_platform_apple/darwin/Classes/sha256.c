/**
 * sha256.c — Minimal SHA-256 and HMAC-SHA256 implementation.
 *
 * Based on FIPS 180-4. Self-contained, no external dependencies.
 */
...
void sha256_update(Sha256Context *ctx, const uint8_t *data, size_t len) {
    size_t buf_pos = (size_t)(ctx->count % SHA256_BLOCK_SIZE);

    ctx->count += len;

    /* If buffer has partial data, fill it first */
    if (buf_pos > 0) {
        size_t space = SHA256_BLOCK_SIZE - buf_pos;
        if (len < space) {
